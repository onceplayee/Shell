...
# 批量删除指定虚拟机
delete_vm() {
    # 调用get_vm_state函数获取虚拟机状态
    get_vm_state
    # 检查上一条命令的退出状态码，如果为23，表示没有虚拟机
    if [ $? -eq 23 ]; then
        # 输出错误信息，提示没有虚拟机
        echo_red "您还没有任何虚拟机存在.无法执行删除操作"
        # 继续下一次循环
